...
LDA_Operator = SDA_Operation(Wav_train_N_dict,Wav_train_V_dict,a,b)

''' Statistics computation '''
print("Statistics construction...")
RedWavSegment_Train_Safe = LDA_Application(LDA_Operator,Wav_train_N_dict)
RedWavSegment_Train_Danger = LDA_Application(LDA_Operator,Wav_train_V_dict)
...
Stat_dict_train = StatisticsConstruction(ReducedMean,ReducedCov,RedWavSegment_Train)
UCL = UCL_Computation(len(ECG_train_N_dict),alpha)

Stat_test_list = list()
marker_list = list()
for idx, key in enumerate(sorted(Stat_dict_test)):
    Stat_test_list.append(Stat_dict_test[key])
    if ECG_test_label_dict[key] == 'N':
        marker_list.append('o')
    elif ECG_test_label_dict[key] == 'V':
        marker_list.append('+')

plt.figure(1, figsize=(12,12))

ax1 = plt.subplot(111)
domain = range(len(Stat_test_list))
# ax1.plot(Stat_test_list,'bo')
switch_normal = True
switch_PVC = True
for idx,key in enumerate(sorted(Stat_dict_test)):
    if ECG_test_label_dict[key] == 'N':
        if switch_normal:
            ax1.scatter(idx, Stat_dict_test[key],marker='o',c='blue',label='Normal')
            switch_normal = False
        else:
            ax1.scatter(idx, Stat_dict_test[key],marker='o',c='blue')
    elif ECG_test_label_dict[key] == 'V':
        if switch_PVC:
            ax1.scatter(idx, Stat_dict_test[key],marker='+',c='red',label='PVC')
            switch_PVC = False
        else:
            ax1.scatter(idx, Stat_dict_test[key],marker='+',c='red')
ax1.axhline(y=UCL,xmin=0,xmax=len(Stat_test_list),c='green',linewidth=2., label="UCL at alpha=0.01")
plt.legend(fontsize=20)
plt.ylabel('T2 statistics', fontsize=20)
plt.xlabel('Index of ECG beats',fontsize=20)
plt.xlim(0,len(Stat_dict_test))
plt.ylim(0,35)

plt.savefig('Fig_SPC.eps',format='eps')
# plt.show()