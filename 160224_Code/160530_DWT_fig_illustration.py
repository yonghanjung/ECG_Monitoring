...
cA4, cD4, cD3, cD2, cD1 = Normal_wc_1
Wavelet_threshold_N1 = np.sqrt(2 * np.log(256)) * (np.median(np.abs(cD1 - np.median(cD1))) / 0.6745)
Normal_wc_1 = map(lambda x: pywt.thresholding.soft(x,Wavelet_threshold_N1), Normal_wc_1)

PVC_wc_1 = pywt.wavedec(data=PVC_1,wavelet=db8,mode='per',level=4 )
cA4, cD4, cD3, cD2, cD1 = PVC_wc_1
Wavelet_threshold_V1 = np.sqrt(2 * np.log(256)) * (np.median(np.abs(cD1 - np.median(cD1))) / 0.6745)
PVC_wc_1 = map(lambda x: pywt.thresholding.soft(x,Wavelet_threshold_V1), PVC_wc_1)


Normal_wc_2 = pywt.wavedec(data=Normal_2,wavelet=db8,mode='per',level=4 )
cA4, cD4, cD3, cD2, cD1 = Normal_wc_2
Wavelet_threshold_N2 = np.sqrt(2 * np.log(256)) * (np.median(np.abs(cD1 - np.median(cD1))) / 0.6745)
Normal_wc_2 = map(lambda x: pywt.thresholding.soft(x,Wavelet_threshold_N2), Normal_wc_2)


''' Wavelet decomposition after denoising '''
# Normal_wc_1 = np.concatenate(Normal_wc_1)
# PVC_wc_1 = np.concatenate(PVC_wc_1)
# Normal_wc_2 = np.concatenate(Normal_wc_2)
#
#
# ax_N1 = ax.add_subplot(3,1,1)
# ax_N1.vlines(np.linspace(0,len(Normal_wc_1),len(Normal_wc_1)),[0],Normal_wc_1)
# plt.axhline(y=0.0,c='black')
#
# ax_V1 = ax.add_subplot(3,1,2)
# ax_V1.vlines(np.linspace(0,len(PVC_wc_1),len(PVC_wc_1)),[0],PVC_wc_1)
# plt.axhline(y=0.0,c='black')
#
# ax_N2 = ax.add_subplot(3,1,3)
# ax_N2.vlines(np.linspace(0,len(Normal_wc_2),len(Normal_wc_2)),[0],Normal_wc_2)
# plt.axhline(y=0.0,c='black')

##################################################

Wavelet_Coef = [Normal_wc_1,PVC_wc_1,Normal_wc_2]


for idx in range(1,16):
    plt.figure(idx,figsize=(10,5))
    ax_idx = plt.subplot(1,1,1)
    sig_idx = (idx-1) % 3
    Target_signal = Wavelet_Coef[sig_idx] # 0,1,2
    # ax_idx = ax.add_subplot(5,3,idx)
    # ax_idx = ax.add_subplot(1,1,1)
    decomposition_index = (idx-1) / 3

    if decomposition_index == 0:
        domain_start = 0
    else:
        domain_start = 16 * (2**(decomposition_index - 1))
    domain_end = 16 * (2**decomposition_index)

    ax_idx.vlines(np.linspace(domain_start, domain_end, domain_end-domain_start),[0],Target_signal[decomposition_index]) #0,0,0,1,1,1,2,2,2,3,3,3,4,4,4
    # ax_idx.yaxis.set_visible(False)
    plt.xlim(domain_start, domain_end)
    plt.axhline(y=0.0, c='black')
    plt.xticks(fontsize=33)
    plt.yticks(fontsize=33)
    plt.tight_layout()
    plt.savefig('Fig_wc_comparison_denoised'+str(idx)+'.eps',format='eps')
    print sig_idx, decomposition_index, len(Target_signal[decomposition_index])
