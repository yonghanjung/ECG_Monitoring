...
WC_Normal_1 = Wav_train_dict[Normal_1_idx]
WC_PVC_1 = Wav_train_dict[PVC_1_idx]
WC_Normal_2 = Wav_train_dict[Normal_2_idx]


sparse_discriminant_vector = SDA_Operation(Wav_train_N_dict,Wav_train_V_dict,a,b)
chosen_param_idx = list()
chosen_WC_N1 = list()
chosen_WC_V1 = list()
chosen_WC_N2 = list()

max_chosen_idx = 0
for idx in range(len(sparse_discriminant_vector)):
    if sparse_discriminant_vector[idx] != 0:
        chosen_WC_N1.append(WC_Normal_1[idx])
        chosen_WC_V1.append(WC_PVC_1[idx])
        chosen_WC_N2.append(WC_Normal_2[idx])
        max_chosen_idx = idx
    else:
        chosen_WC_N1.append(0.0)
        chosen_WC_V1.append(0.0)
        chosen_WC_N2.append(0.0)



ax = plt.figure(1,figsize=(10,5))
ax_N1 = ax.add_subplot(1,1,1)
ax_N1.vlines(np.linspace(0,len(WC_Normal_1), len(WC_Normal_1)),[0], chosen_WC_N1  )
# ax_N1.yaxis.set_visible(False)
ax_N1.set_xlim(0,max_chosen_idx+5)
...
ax = plt.figure(2,figsize=(10,5))
ax_V1 = ax.add_subplot(1,1,1)
ax_V1.vlines(np.linspace(0,len(WC_PVC_1), len(WC_PVC_1)),[0], chosen_WC_V1  )
# ax_V1.yaxis.set_visible(False)
ax_V1.set_xlim(0,max_chosen_idx+5)
plt.axhline(y=0.0, c='black')
plt.xticks(fontsize=33)
plt.yticks(fontsize=33)
plt.tight_layout()
plt.savefig('Fig_chosen-WC-SDA_V1.eps',format='eps')

ax = plt.figure(3,figsize=(10,5))
ax_N2 = ax.add_subplot(1,1,1)
ax_N2.vlines(np.linspace(0,len(WC_Normal_1), len(WC_Normal_1)),[0], chosen_WC_N2  )
# ax_N2.yaxis.set_visible(False)
ax_N2.set_xlim(0,max_chosen_idx+5)
plt.axhline(y=0.0, c='black')
plt.xticks(fontsize=33)
plt.yticks(fontsize=33)
plt.tight_layout()
plt.savefig('Fig_chosen-WC-SDA_N2.eps',format='eps')
